...
def test_run_one():
    ...
    assert row[1] == STATUS.ok, f"Error, the notebooks produces {STATUS[row[1]]}!=ok"

    # run and fail
    row, errors = run_one(FAIL_NB)
    assert errors is not None, f"Notebook {FAIL_NB} has no errors, it should!"
    assert row[1] == STATUS.fail, f"Error, the notebooks produces {STATUS[row[1]]}!=fail"

    # s kipped
    row, no_errors = run_one(SKIP_NB)
    assert no_errors is None, f"Notebook {TEST_NB} failed"
    assert row[1] == STATUS.skip, f"Error, the notebooks produces {STATUS[row[1]]}!=skip"
